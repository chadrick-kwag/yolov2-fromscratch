...
class InputLoader():
    ...
    def get_image_and_gt(self):
        ...
        if not picked_files:
            ...
            if not picked_files:
                raise Exception(
                    "failed to reload annotation list and re-fetch batch_num number of annotations")
        ...
        for f in picked_files:
            resized_image, gt, essence = self.process_single_annotation_file(
                f, return_gt=True)
            gt_batch.append(gt)
    ...
    def reload_annotation_file_list(self):
        ...
        self.annotation_file_list = [
            f for f in temp_annotation_file_list if self.check_if_json_file(f)]
    ...
    def process_gt_from_annotation_info(self, jsonobj, grid_slice_num=13):
        ...
        for single_object in obj_list:
            ...
            best_B_index = self.apmanager.best_matching_ap_index(
                resized_bw, resized_bh)
            ...
            center_xy_grid_index = int(
                grid_y_index * grid_slice_num + grid_x_index)
            ...
            returnarray = np.zeros(
                shape=(number_of_grids, number_of_boxes, number_of_predictions_for_each_box))
            ...
            returnarray[center_xy_grid_index, best_B_index, :] = [
                r_cx, r_cy, resized_bw, resized_bh, 1.0, 1.0]
            ...
            essence = list()
            essence.append([center_xy_grid_index, best_B_index,
                            r_cx, r_cy, resized_bw, resized_bh])
